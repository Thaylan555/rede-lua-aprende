...
export function GameView({ initialCode = "" }: { initialCode?: string }) {
  ...
  const bossMode = game.experienceMode === "boss_battle" || templatePreset === "chefao";
  const treasureMode = game.experienceMode === "treasure_hunt" || templatePreset === "tesouro";
  const spaceRaceMode = game.experienceMode === "space_race";
  const cardDuelMode = game.experienceMode === "card_duel";
  ...
  return <div className={`game-stage themed-game mode-${game.experienceMode} pattern-${theme.pattern} buttons-${theme.buttonShape} font-${theme.fontStyle}`} style={stageStyle}>
    ...
    {game.status === "running" && game.question && <main className="page-width question-stage branded-question-stage">
      ...
      {bossMode ? <div className="boss-battle-panel"><div><span>👾 CHEFÃO DA TURMA</span><strong>{bossEnergy}% de energia</strong></div><div className="boss-health"><i style={{ width:`${bossEnergy}%` }} /></div><small>Cada pergunta vencida empurra a turma mais perto da vitória.</small></div> : game.experienceMode === "lunar_rush" && <div className="mode-banner rush"><Zap /><span>Corrida Lunar: responda cedo para ganhar bônus de velocidade.</span></div>}
      {treasureMode ? <div className="treasure-progress"><span>🗺️ PISTA {treasureStep}/{game.questionCount}</span><div>{Array.from({length:Math.min(game.questionCount,8)},(_,i)=><i key={i} className={i < Math.ceil((treasureStep / Math.max(1,game.questionCount))*Math.min(game.questionCount,8)) ? "found" : ""} />)}</div><small>Responda para revelar o próximo pedaço do mapa.</small></div> : game.experienceMode === "star_hunt" && <div className="mode-banner stars"><Star /><span>{stars} estrela{stars === 1 ? "" : "s"} coletada{stars === 1 ? "" : "s"} nesta partida.</span></div>}
      {spaceRaceMode && <div className="mode-banner rush"><Zap/><span>Corrida Espacial: cada acerto empurra sua nave mais perto da chegada.</span></div>}
      {cardDuelMode && <div className="mode-banner stars"><Gamepad2/><span>Duelo de Cartas: cada pergunta é uma rodada do duelo.</span></div>}
    ...
    {game.status === "finished" && <main className="page-width game-center"><section className="result-card branded-result"><Medal /><span>PARTIDA FINALIZADA</span><h1>{bossMode ? "Chefão derrotado!" : treasureMode ? "Tesouro encontrado!" : spaceRaceMode ? "Linha de chegada cruzada!" : cardDuelMode ? "Duelo encerrado!" : "Missão concluída."}</h1><p>Você terminou com <strong>{game.participant.score} pontos</strong>.</p>{game.experienceMode === "star_hunt" && <div className="final-stars"><Star /> {stars} estrelas coletadas</div>}<div className="leaderboard-final">{game.leaderboard.map((p, i) => <div className={p.id === game.participant.id ? "me" : ""} key={p.id}><b>{i + 1}</b><span>{p.displayName}</span><strong>{p.score} pts</strong></div>)}</div><button className="button button-primary" onClick={() => { sessionStorage.removeItem(`rede-lua-game:${normalizedCode}`); setParticipant(null); setCode(""); }}>Entrar em outra partida</button></section></main>}
  </div>;
}

function modeLabel(mode: string) { return mode === "lunar_rush" ? "Corrida Lunar" : mode === "star_hunt" ? "Caça às Estrelas" : mode === "focus" ? "Modo Foco" : mode === "boss_battle" ? "Batalha de Chefão" : mode === "treasure_hunt" ? "Caça ao Tesouro" : mode === "space_race" ? "Corrida Espacial" : mode === "card_duel" ? "Duelo de Cartas" : "Quiz Clássico"; }

...
export function GameView({ initialCode = "" }: { initialCode?: string }) {
  ...
  return <div className={`game-stage themed-game mode-${game.experienceMode} pattern-${theme.pattern} buttons-${theme.buttonShape} font-${theme.fontStyle}`} style={stageStyle}>
    ...
    {game.status === "lobby" && <main className="page-width game-center"><section className="lobby-card branded-lobby"><div className="pulse-moon">{theme.logoUrl ? <img src={theme.logoUrl} alt="" /> : theme.showRedeLuaBrand ? <img src="/assets/rede-lua/brand/logo-transparent.webp" alt="" /> : <MoonStar />}</div><span>VOCÊ ENTROU • {modeLabel(game.experienceMode).toUpperCase()}</span><h1>Aguardando o professor começar.</h1><p>O visual, o tempo e o formato desta partida foram escolhidos pelo professor.</p><ReactionDock reaction={reaction} setReaction={setReaction} /><div className="leader-mini"><UsersLabel count={game.leaderboard.length} />{game.leaderboard.slice(0, 5).map((p, i) => <span key={p.id}><b>{i + 1}</b>{p.displayName}</span>)}</div></section></main>}
...
function ReactionDock({ reaction, setReaction, compact = false }: { reaction: string | null; setReaction: (value: string) => void; compact?: boolean }) { return <div className={`reaction-dock ${compact ? "compact" : ""}`}>{reactionAssets.slice(0, compact ? 5 : 8).map((item) => <button key={item.id} className={reaction === item.id ? "active" : ""} onClick={() => setReaction(item.id)} title={item.label}><img src={item.src} alt={item.label} /></button>)}</div>; }
